  ...
  useEffect(() => {
    if (id) {
      const fetchItem = async () => {
        try {
          const res = await fetch(`/api/WomenCareusel/${id}`)
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const res = await fetch(
      id ? `/api/WomenCareusel/${id}` : '/api/WomenCareusel',
      {
        method: id ? 'PUT' : 'POST',
        body: formData
      }
    ...
    if (data.success) {
      ...
      router.push('/careusel/womenCareusel') 
    } else {
  ...
  return (
    <Card>
      <CardHeader title={id ? 'Edit Women Careusel' : 'Create Women Careusel'} />
      <CardContent>
            ...
            <Grid item xs={12}>
              <div className='flex max-sm:flex-col items-center gap-6'>
                <Grid item xs={4} sm={3} md={3}>
                  <img className="rounded w-full h-auto" src={imgSrc} alt="Women Careusel" />
                </Grid>
                <div className='flex flex-grow flex-col gap-4'>
                  <div className='flex flex-col sm:flex-row gap-4'>
                    <Button component='label' size='small' variant='contained' htmlFor='women-careusel-upload-image'>
                      Upload New Photo
                      <input
                        hidden
                        type='file'
                        ...
                        id='women-careusel-upload-image'
                      />
                    </Button>
...
export default WomenCareuselForm